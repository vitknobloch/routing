...
CvrpSAStep::CvrpSAStep() : rand_(), gen_(rand_()), dist_(0.0, 1.0){
...
CvrpSAStep::neighborhood_options CvrpSAStep::selectNeighborhoodOption() {
  double max_val = 0;
...
void CvrpSAStep::success(CvrpSAStep::neighborhood_options neighborhood) {
  try_count[neighborhood] += 1;
  success_count[neighborhood] += 1;
}

void CvrpSAStep::failure(CvrpSAStep::neighborhood_options neighborhood) {
  try_count[neighborhood] += 1;
}

void CvrpSAStep::reset(const std::shared_ptr<Individual> &individual) {
  for(int i = 0; i < neighborhood_options::SIZE; i++){
    try_count[i] = 1;
    success_count[i] = 1;
  }
...
StepResult
CvrpSAStep::step(const std::shared_ptr<Individual> &individual,
                 const std::shared_ptr<SASchedule> &schedule,
                 const std::shared_ptr<Individual> &best_individual) {
  const auto &individual_ = std::static_pointer_cast<CvrpIndividualStructured>(individual);
  const auto &best_individual_ = std::static_pointer_cast<CvrpIndividualStructured>(best_individual);
...
StepResult CvrpSAStep::perform2opt(
    const std::shared_ptr<CvrpIndividualStructured> &individual,
    const std::shared_ptr<SASchedule> &schedule,
    const std::shared_ptr<CvrpIndividualStructured> &best_individual) {
  ...
  if(diff.constraints < 0 || (diff.constraints == 0 && diff.fitness <= 0)){
...
StepResult CvrpSAStep::performExchange(
    const std::shared_ptr<CvrpIndividualStructured> &individual,
    const std::shared_ptr<SASchedule> &schedule,
    const std::shared_ptr<CvrpIndividualStructured> &best_individual) {
...
StepResult CvrpSAStep::performRelocate(
    const std::shared_ptr<CvrpIndividualStructured> &individual,
    const std::shared_ptr<SASchedule> &schedule,
    const std::shared_ptr<CvrpIndividualStructured> &best_individual) {
  uint route1_idx = randomSelectRoute(individual, 1);
...
StepResult CvrpSAStep::performCross(
    const std::shared_ptr<CvrpIndividualStructured> &individual,
    const std::shared_ptr<SASchedule> &schedule,
    const std::shared_ptr<CvrpIndividualStructured> &best_individual) {
  uint route1_idx = randomSelectRoute(individual, 1);
...
uint CvrpSAStep::randomSelectRoute(
    const std::shared_ptr<CvrpIndividualStructured> &individual,
...
CvrpRouteSegment CvrpSAStep::randomSelectSegment(
    const std::shared_ptr<CvrpIndividualStructured> &individual, uint route_idx,
