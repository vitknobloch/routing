...
VrptwPmxCrossoverStructured::VrptwPmxCrossoverStructured() : crossover_rate_(1.0), gen_(rand()){

}

void VrptwPmxCrossoverStructured::setCrossoverRate(double crossover_rate) {
  crossover_rate_ = crossover_rate;
}
std::shared_ptr<Population> VrptwPmxCrossoverStructured::crossover(
  ...
  const auto &representative_ = std::static_pointer_cast<VrptwIndividualStructured>(representative);
  std::uniform_int_distribution<uint> distribution(0, representative_->flatten().size() - 1);

  for(uint p = 0; p < parents.size() - 1; p++){
    const auto &parent1 = std::static_pointer_cast<VrptwIndividualStructured>(population->getIndividual((int)parents[p]));
    const auto &parent2 = std::static_pointer_cast<VrptwIndividualStructured>(population->getIndividual((int)parents[p+1]));
...
double VrptwPmxCrossoverStructured::getCrossoverRate() { return crossover_rate_; }
std::shared_ptr<VrptwIndividualStructured>
VrptwPmxCrossoverStructured::crossoverTwo(
  ...
  auto child = std::make_shared<VrptwIndividualStructured>(*parent1, data);
